...
st.markdown(f"<h6 style='color: green;'>Selected Date : {date}</h6>", unsafe_allow_html=True)
...
start_datetime = pd.to_datetime(f"{date}")
...
for model_file in os.listdir():
    try:
        ...
        forecast_datetime = pd.date_range(start=start_datetime, periods=future_steps+1, freq='D')[1:]
...
price_fit = price_pred.fit()
price_forecast = price_fit.forecast(steps=7)

st.markdown(f"<h4 style='background-color:violet;'>Predicted Profits for next week:</h4>", unsafe_allow_html=True)
st.markdown(f"<h5 align='center'>Predicted Profit: {round(price_forecast.sum(), 2)}</h5>", unsafe_allow_html=True)
